...
impl TokenUsage {
    pub fn new() -> Self { Self { input: 0, output: 0, max: 200_000 } }
}
...
impl App {
    ...
    async fn handle_event(
        ...
        match self.status.clone() {
            ...
            Status::SessionBrowser => match key.code {
                ...
                KeyCode::Down  => { let max = self.session_list.len().saturating_sub(1); self.session_selected = (self.session_selected + 1).min(max); }
                KeyCode::Enter => self.load_selected_session(),
                KeyCode::Esc   => { self.status = Status::Ready; }
                _ => {}
            },
            ...
            _ => match key.code {
                ...
                KeyCode::Up       => { self.auto_scroll = false; self.scroll = self.scroll.saturating_sub(3); }
                KeyCode::Down     => {
                    self.scroll = self.scroll.saturating_add(3);
                    if self.scroll >= u16::MAX - 3 { self.auto_scroll = true; self.scroll = u16::MAX; }
                }
                _ => {}
            },
        }
    ...
    fn handle_setup_key(&mut self, code: KeyCode, step: SetupStep) {
        match step {
            SetupStep::ChooseProvider => match code {
                KeyCode::Up | KeyCode::Char('k') => { self.setup_selected = self.setup_selected.saturating_sub(1); }
                KeyCode::Down | KeyCode::Char('j') => { self.setup_selected = (self.setup_selected + 1).min(PROVIDERS.len() - 1); }
                KeyCode::Char('1') => { self.setup_selected = 0; self.advance_setup(); }
                KeyCode::Char('2') => { self.setup_selected = 1; self.advance_setup(); }
                KeyCode::Char('3') => { self.setup_selected = 2; self.advance_setup(); }
    ...
    fn resolve_permission(&mut self, always: bool, tx: mpsc::Sender<StreamEvent>) {
        ...
        tokio::spawn(async move {
            let output = tokio::task::spawn_blocking(move || {
                tools::execute(&call.name, call.input.clone()).unwrap_or_else(|e| format!("error: {e}"))
            })
            .await
    ...
    fn deny_permission(&mut self, tx: mpsc::Sender<StreamEvent>) {
        ...
        let _ = tx.try_send(StreamEvent::ToolResult { id, output: "user denied this tool call".to_string() });
    }
    ...
    fn interrupt(&mut self) {
        ...
        if !text.is_empty() { self.messages.push(Message::assistant(text + " [interrupted]")); }
        self.pending_calls.clear();
        self.current_call_idx = 0;
        self.status = Status::Ready;
    }
    ...
    fn handle_stream(&mut self, event: StreamEvent, tx: mpsc::Sender<StreamEvent>) {
        match event {
            ...
            StreamEvent::ToolCalls { calls, accumulated_text, input_tokens, output_tokens } => {
                ...
                self.streaming_text.clear();
                self.pending_calls = calls.into_iter()
            ...
            StreamEvent::ToolResult { id, output } => {
                if let Some(c) = self.pending_calls.iter_mut().find(|c| c.id == id) { c.result = Some(output.clone()); }
                let name = self.pending_calls.iter().find(|c| c.id == id).map(|c| c.name.clone()).unwrap_or_default();
                self.diff_content = format!("tool: {name}\n\n{output}");
                self.advance_tool_queue(tx);
            }
            StreamEvent::Done { input_tokens, output_tokens } => {
                let text = std::mem::take(&mut self.streaming_text);
                if !text.is_empty() { self.messages.push(Message::assistant(text)); }
                self.tokens.input = input_tokens;
                self.tokens.output = output_tokens;
                self.status = Status::Ready;
                self.stream_task = None;
            ...
            StreamEvent::Error(e) => {
                ...
                if !leftover.is_empty() { self.messages.push(Message::assistant(leftover)); }
...
fn make_provider(name: &str, key: &str, model: &str) -> Arc<dyn Provider> {
    match name {
        "openai" => Arc::new(OpenAIProvider::new(key.to_string(), model.to_string())),
